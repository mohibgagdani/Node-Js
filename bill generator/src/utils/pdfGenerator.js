...
export const generatePDF = async (invoiceData, templateNumber, downloadOnly = true) => {
...
export const sendBillViaEmail = async (invoiceData, templateNumber, email) => {
  try {
    const { pdf, fileName } = await generatePDF(invoiceData, templateNumber, false);
    const pdfBlob = pdf.output('blob');
    
    const formData = new FormData();
    formData.append('pdfFile', pdfBlob, fileName);
    formData.append('email', email);
    formData.append('billData', JSON.stringify(invoiceData));
